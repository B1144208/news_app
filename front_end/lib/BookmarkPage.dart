...
class _BookmarkPageState extends State<BookmarkPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true && data['result'] != null) {
          ...
          // 按照順序排序
          categories.sort(
            (a, b) => (a['groupcustomize_order'] ?? 0).compareTo(
              b['groupcustomize_order'] ?? 0,
            ),
          );
  ...
  Future<void> _fetchBookmarkedData() async {
    ...
    try {
      await Future.wait([_fetchBookmarkedNews(), _fetchBookmarkedChannels()]);
    } catch (e) {
  ...
  Future<void> _fetchBookmarkedNews() async {
    try {
      final url =
          '$baseUrl/api/user_action/bookmark/news?userId=$_currentUserId';
  ...
  Future<void> _fetchBookmarkedChannels() async {
    try {
      final url =
          '$baseUrl/api/user_action/bookmark/channel?userId=$_currentUserId';
      final response = await http.get(Uri.parse(url));
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Icon(Icons.menu, color: Color(0xFF60a5fa), size: 20),
            ),
          ),
                ...
                child: Center(
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight:
                          _showNews ? FontWeight.bold : FontWeight.normal,
                    ),
                  ),
                ),
              ),
  ...
  Widget _buildContentList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _showNews ? _buildNewsItem(item) : _buildChannelItem(item);
      },
    );
  }
