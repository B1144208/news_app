  ...
  Future<void> _fetchUserCategories() async {
    ...
    try {
      final groupResponse = await http.get(
        Uri.parse('${Config.apiBaseUrl}/groupcustomize/order?type=general&userId=${widget.userId}'),
      );
  ...
  Future<void> _executeAddCategory(String newName) async {
    ...
    try {
      ...
      if (createGroupData['id'] == null) {
        throw Exception(createMessage.isNotEmpty ? createMessage : '建立基礎分類失敗: ID欄位遺失');
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('分類 "$newName" 已成功新增並加入列表！'), backgroundColor: Colors.green),
        );
      ...
      if (errorString.contains('insert success') ||
          ...
          errorString.contains('api response successful but missing new group id')) {
        if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('新增分類失敗: $error'), backgroundColor: Colors.red),
        );
      }
    } finally {
      if (mounted) {
            ...
            TextButton(
              ...
              onPressed: () {
                ...
                if (newName.isNotEmpty) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('分類名稱不能為空'), backgroundColor: Colors.red),
                  );
                }
              },
            ),
          ...
          Expanded(
            child: ReorderableListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                Key itemKey = ObjectKey({'id': category['group_id'], 'order': category['group_order']});
