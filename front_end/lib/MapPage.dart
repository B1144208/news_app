...
class _MapPageState extends State<MapPage> {
  ...
  Widget _buildRightPanel() {
    ...
    final String countryName =
        _selectedLocation!['country_name_zh_tw'] ?? '未知國家';
    ...
    final bool hasRegionScope =
        _safeId('region_id', _selectedLocation!) != null;
  ...
  void _setMapToLastLocation(int id, String idKey) {
    try {
      ...
      if (targetLocation != null) {
        ...
        final stateLat = double.tryParse(
          targetLocation['state_center_latitude']?.toString() ?? '',
        );
        final stateLng = double.tryParse(
          targetLocation['state_center_longitude']?.toString() ?? '',
        );
        final countryLat = double.tryParse(
          targetLocation['country_center_latitude']?.toString() ?? '',
        );
        final countryLng = double.tryParse(
          targetLocation['country_center_longitude']?.toString() ?? '',
        );
  ...
  void _handleDropdownSearch() {
    ...
    if (_selectedState != null && _selectedState != _kUnselectOption) {
      ...
      countryIdToSave =
          targetLocation != null ? _safeId('country_id', targetLocation) : null;
    } else if (_selectedCountry != null &&
        _selectedCountry != _kUnselectOption) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('地點已清除，定位至世界中心。')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.bookmark, color: Color(0xFF60a5fa)),
            onPressed: () {
              // 假設 BookmarkPage 存在
              // Navigator.push(
              //   context,
      ...
      body: Column(
        children: [
          ...
          if (!_isLoading && _error == null)
            Column(children: [_buildDropdownFilters(), _buildSearchButton()]),
