...
class _MapPageState extends State<MapPage> {
  ...
  // 💥 新增：設置當前地點並獲取新聞
  void _setCurrentLocationAndFetchNews(Map<String, dynamic> locationData, String scope) {
    ...
    if (idToUse != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('地點資料不完整，無法取得新聞。'))
      );
    }
  }
  ...
  void _searchLocation(String query) {
    ...
    if (foundLocation != null) {
      ...
      final String scopeToUse = _safeId('state_id', foundLocation) != null ? 'state' :
      _safeId('country_id', foundLocation) != null ? 'country' : 'region';
  ...
  void _handleDropdownSearch() {
    ...
    if (_selectedState != null && _selectedState != _kUnselectOption) {
      ...
      _loadDefaultNews();
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('地點已清除，定位至預設新聞。'))
      );
      return;
    }
    ...
    if (targetLocation != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('找不到該地點的完整資料。'))
      );
    }
  }
          ...
          builder: (BuildContext context, StateSetter setStateInDialog) {
            ...
            return AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('或使用下拉選單定位：', style: TextStyle(fontWeight: FontWeight.bold)),
              ...
              actions: <Widget>[
                ...
                ElevatedButton(
                  onPressed: (_selectedRegion != null && _selectedRegion != _kUnselectOption) ? _handleDropdownSearch : null,
  ...
  void _navigateToFullMapSearch() async {
    final Map<String, dynamic>? resultLocation = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => _FullMapSearchPage(
  ...
  Widget _buildNewsList() {
    ...
    final String countryName = _currentNewsLocation!['country_name_zh_tw'] ?? '未知國家';
    final String stateName = _currentNewsLocation!['state_name_en'] ?? '未提供';
    final String regionName = _currentNewsLocation!['region_name_zh_tw'] ?? '未知地區';
    ...
    final bool hasStateScope = _safeId('state_id', _currentNewsLocation!) != null;
    final bool hasRegionScope = _safeId('region_id', _currentNewsLocation!) != null;
          ...
          child: _newsList.isEmpty
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  leading: news.coverImage != null
                      ? Image.network(news.coverImage!, width: 50, height: 50, fit: BoxFit.cover,
                    errorBuilder: (context, error, stackTrace) => const Icon(Icons.image, color: Colors.grey),
                  )
                      : const Icon(Icons.article, color: Colors.blue),
                  title: Text(news.title, maxLines: 2, overflow: TextOverflow.ellipsis),
                  subtitle: Text('${news.sourceName ?? '未知'} · ${news.publishDate ?? ''}'),
...
class _FullMapSearchPageState extends State<_FullMapSearchPage> {
  ...
  void _initializeMap() {
    if (widget.initialLocation != null) {
      ...
      final stateLat = double.tryParse(loc['state_center_latitude']?.toString() ?? '');
      final stateLng = double.tryParse(loc['state_center_longitude']?.toString() ?? '');
      final countryLat = double.tryParse(loc['country_center_latitude']?.toString() ?? '');
      final countryLng = double.tryParse(loc['country_center_longitude']?.toString() ?? '');
  ...
  void _handleMapTap(TapPosition tapPosition, LatLng latlng) {
    ...
    if (nearestLocation != null) {
      ...
      final stateLng = double.tryParse(nearestLocation['state_center_longitude']?.toString() ?? '');
      final countryLat = double.tryParse(nearestLocation['country_center_latitude']?.toString() ?? '');
      final countryLng = double.tryParse(nearestLocation['country_center_longitude']?.toString() ?? '');
      ...
      if (stateId != null && stateLat != null && stateLng != null) {
        ...
        lng = stateLng;
        zoomLevel = 9.0;
      } else if (countryId != null && countryLat != null && countryLng != null) {
        lat = countryLat;
        lng = countryLng;
        zoomLevel = 7.0;
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('該地點沒有可用的經緯度資料進行定位。'))
        );
        return;
      }
