...
-- 程序
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_reset_heat_totals` ()   BEGIN
  UPDATE news_data                 SET total_heat = 0;
  UPDATE channel_data              SET total_heat = 0;
  UPDATE eventsorting_data         SET total_heat = 0;
  UPDATE multipleperspectives_data SET total_heat = 0;
  UPDATE keyword_data              SET total_search_heat = 0;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_rollup_recent` ()   BEGIN
  	DECLARE k DOUBLE;
  	-- k ≈ 0.083214... ：36 小時衰減到 5%
  	SET k = -LN(0.05)/36;
    ...
    ======================= */
  	...
  	/* view */
  	LEFT JOIN (
        SELECT uv.news_id AS id,
			SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uv.created_at, NOW())))) AS cnt
        FROM recent_action ra
        JOIN user_view uv ON uv.view_id = ra.view_id
        WHERE uv.news_id IS NOT NULL
        GROUP BY uv.news_id
  	) v   ON v.id = nd.news_id
  	/* share */
  	LEFT JOIN (
        SELECT us.news_id AS id,
			SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, us.created_at, NOW())))) AS cnt
        FROM recent_action ra
        JOIN user_share us ON us.share_id = ra.share_id
        WHERE us.news_id IS NOT NULL
        GROUP BY us.news_id
  	) sh  ON sh.id = nd.news_id
  	/* search */
  	LEFT JOIN (
        SELECT ux.news_id AS id,
			SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ux.created_at, NOW())))) AS cnt
        FROM recent_action ra
        JOIN user_search ux ON ux.search_id = ra.search_id
        WHERE ux.news_id IS NOT NULL
        GROUP BY ux.news_id
	) sr  ON sr.id = nd.news_id
  	/* bookmark */
  	LEFT JOIN (
        SELECT ub.news_id AS id,
			SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ub.created_at, NOW())))) AS cnt
        FROM recent_action ra
        JOIN user_bookmark ub ON ub.bookmark_id = ra.bookmark_id
        WHERE ub.news_id IS NOT NULL
        GROUP BY ub.news_id
  	) bm  ON bm.id = nd.news_id
	/* comment */
    LEFT JOIN (
        SELECT uc.news_id AS id,
			SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uc.created_at, NOW())))) AS cnt
        FROM recent_action ra
        JOIN user_comment uc ON uc.comment_id = ra.comment_id
        WHERE uc.news_id IS NOT NULL
        GROUP BY uc.news_id
    ) cm  ON cm.id = nd.news_id
  	/* score */
  	LEFT JOIN (
        SELECT usco.news_id AS id,
        	SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, usco.created_at, NOW())))) AS cnt
        FROM recent_action ra
        JOIN user_score usco ON usco.score_id = ra.score_id
        WHERE usco.news_id IS NOT NULL
        GROUP BY usco.news_id
  	) sc  ON sc.id = nd.news_id
  	SET nd.total_recent_view      = ROUND(COALESCE(v.cnt, 0)),
		nd.total_recent_share     = ROUND(COALESCE(sh.cnt, 0)),
        nd.total_recent_search    = ROUND(COALESCE(sr.cnt, 0)),
        nd.total_recent_bookmark  = ROUND(COALESCE(bm.cnt, 0)),
        nd.total_recent_comment   = ROUND(COALESCE(cm.cnt, 0)),
        nd.total_recent_score     = ROUND(COALESCE(sc.cnt, 0));

  /* =======================
     2) channel_data
     ======================= */
  UPDATE channel_data cd
  LEFT JOIN (
    SELECT uv.channel_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uv.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_view uv ON uv.view_id = ra.view_id
    WHERE uv.channel_id IS NOT NULL
    GROUP BY uv.channel_id
  ) v   ON v.id = cd.channel_id
  LEFT JOIN (
    SELECT us.channel_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, us.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_share us ON us.share_id = ra.share_id
    WHERE us.channel_id IS NOT NULL
    GROUP BY us.channel_id
  ) sh  ON sh.id = cd.channel_id
  LEFT JOIN (
    SELECT ux.channel_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ux.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_search ux ON ux.search_id = ra.search_id
    WHERE ux.channel_id IS NOT NULL
    GROUP BY ux.channel_id
  ) sr  ON sr.id = cd.channel_id
  LEFT JOIN (
    SELECT ub.channel_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ub.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_bookmark ub ON ub.bookmark_id = ra.bookmark_id
    WHERE ub.channel_id IS NOT NULL
    GROUP BY ub.channel_id
  ) bm  ON bm.id = cd.channel_id
  LEFT JOIN (
    SELECT uc.channel_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uc.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_comment uc ON uc.comment_id = ra.comment_id
    WHERE uc.channel_id IS NOT NULL
    GROUP BY uc.channel_id
  ) cm  ON cm.id = cd.channel_id
  LEFT JOIN (
    SELECT usco.channel_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, usco.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_score usco ON usco.score_id = ra.score_id
    WHERE usco.channel_id IS NOT NULL
    GROUP BY usco.channel_id
  ) sc  ON sc.id = cd.channel_id
  SET cd.total_recent_view      = ROUND(COALESCE(v.cnt, 0)),
      cd.total_recent_share     = ROUND(COALESCE(sh.cnt, 0)),
      cd.total_recent_search    = ROUND(COALESCE(sr.cnt, 0)),
      cd.total_recent_bookmark  = ROUND(COALESCE(bm.cnt, 0)),
      cd.total_recent_comment   = ROUND(COALESCE(cm.cnt, 0)),
      cd.total_recent_score     = ROUND(COALESCE(sc.cnt, 0));

  /* =======================
     3) eventsorting_data
     ======================= */
  UPDATE eventsorting_data ed
  LEFT JOIN (
    SELECT uv.eventsorting_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uv.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_view uv ON uv.view_id = ra.view_id
    WHERE uv.eventsorting_id IS NOT NULL
    GROUP BY uv.eventsorting_id
  ) v   ON v.id = ed.eventsorting_id
  LEFT JOIN (
    SELECT us.eventsorting_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, us.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_share us ON us.share_id = ra.share_id
    WHERE us.eventsorting_id IS NOT NULL
    GROUP BY us.eventsorting_id
  ) sh  ON sh.id = ed.eventsorting_id
  LEFT JOIN (
    SELECT ux.eventsorting_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ux.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_search ux ON ux.search_id = ra.search_id
    WHERE ux.eventsorting_id IS NOT NULL
    GROUP BY ux.eventsorting_id
  ) sr  ON sr.id = ed.eventsorting_id
  LEFT JOIN (
    SELECT ub.eventsorting_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ub.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_bookmark ub ON ub.bookmark_id = ra.bookmark_id
    WHERE ub.eventsorting_id IS NOT NULL
    GROUP BY ub.eventsorting_id
  ) bm  ON bm.id = ed.eventsorting_id
  LEFT JOIN (
    SELECT uc.eventsorting_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uc.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_comment uc ON uc.comment_id = ra.comment_id
    WHERE uc.eventsorting_id IS NOT NULL
    GROUP BY uc.eventsorting_id
  ) cm  ON cm.id = ed.eventsorting_id
  LEFT JOIN (
    SELECT usco.eventsorting_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, usco.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_score usco ON usco.score_id = ra.score_id
    WHERE usco.eventsorting_id IS NOT NULL
    GROUP BY usco.eventsorting_id
  ) sc  ON sc.id = ed.eventsorting_id
  SET ed.total_recent_view      = ROUND(COALESCE(v.cnt, 0)),
      ed.total_recent_share     = ROUND(COALESCE(sh.cnt, 0)),
      ed.total_recent_search    = ROUND(COALESCE(sr.cnt, 0)),
      ed.total_recent_bookmark  = ROUND(COALESCE(bm.cnt, 0)),
      ed.total_recent_comment   = ROUND(COALESCE(cm.cnt, 0)),
      ed.total_recent_score     = ROUND(COALESCE(sc.cnt, 0));

  /* =======================
     4) multipleperspectives_data
     ======================= */
  UPDATE multipleperspectives_data md
  LEFT JOIN (
    SELECT uv.multipleperspectives_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uv.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_view uv ON uv.view_id = ra.view_id
    WHERE uv.multipleperspectives_id IS NOT NULL
    GROUP BY uv.multipleperspectives_id
  ) v   ON v.id = md.multipleperspectives_id
  LEFT JOIN (
    SELECT us.multipleperspectives_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, us.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_share us ON us.share_id = ra.share_id
    WHERE us.multipleperspectives_id IS NOT NULL
    GROUP BY us.multipleperspectives_id
  ) sh  ON sh.id = md.multipleperspectives_id
  LEFT JOIN (
    SELECT ux.multipleperspectives_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ux.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_search ux ON ux.search_id = ra.search_id
    WHERE ux.multipleperspectives_id IS NOT NULL
    GROUP BY ux.multipleperspectives_id
  ) sr  ON sr.id = md.multipleperspectives_id
  LEFT JOIN (
    SELECT ub.multipleperspectives_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, ub.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_bookmark ub ON ub.bookmark_id = ra.bookmark_id
    WHERE ub.multipleperspectives_id IS NOT NULL
    GROUP BY ub.multipleperspectives_id
  ) bm  ON bm.id = md.multipleperspectives_id
  LEFT JOIN (
    SELECT uc.multipleperspectives_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, uc.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_comment uc ON uc.comment_id = ra.comment_id
    WHERE uc.multipleperspectives_id IS NOT NULL
    GROUP BY uc.multipleperspectives_id
  ) cm  ON cm.id = md.multipleperspectives_id
  LEFT JOIN (
    SELECT usco.multipleperspectives_id AS id,
           SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, usco.created_at, NOW())))) AS cnt
    FROM recent_action ra
    JOIN user_score usco ON usco.score_id = ra.score_id
    WHERE usco.multipleperspectives_id IS NOT NULL
    GROUP BY usco.multipleperspectives_id
  ) sc  ON sc.id = md.multipleperspectives_id
  SET md.total_recent_view      = ROUND(COALESCE(v.cnt, 0)),
      md.total_recent_share     = ROUND(COALESCE(sh.cnt, 0)),
      md.total_recent_search    = ROUND(COALESCE(sr.cnt, 0)),
      md.total_recent_bookmark  = ROUND(COALESCE(bm.cnt, 0)),
      md.total_recent_comment   = ROUND(COALESCE(cm.cnt, 0)),
      md.total_recent_score     = ROUND(COALESCE(sc.cnt, 0));

  /* =======================
     5) keyword_data
     ======================= */
    UPDATE keyword_data AS kd
    JOIN (
        SELECT 
            usr.keyword_id,
            SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, usr.created_at, NOW())))) AS total_recent
        FROM user_search_record AS usr
        JOIN recent_search AS rs
            ON rs.record_id = usr.record_id
        GROUP BY usr.keyword_id
    ) AS temp
    ON kd.keyword_id = temp.keyword_id
    SET kd.total_recent_search = temp.total_recent;
    
    /* =======================
     6) relation_data
     ======================= */
	UPDATE relation_data AS rd
    JOIN (
        SELECT 
            nd.relation_id,
            SUM(100 * EXP(-k * CEIL(TIMESTAMPDIFF(HOUR, nd.created_at, NOW())))) AS total_recent
        FROM news_data AS nd
        JOIN recent_news AS rn
            ON rn.news_id = nd.news_id
        GROUP BY nd.relation_id
    ) AS agg
    ON rd.relation_id = agg.relation_id
    SET rd.total_recent_news = agg.total_recent;
    
  /* =======================
     7) 清理：將超過 36 小時的行為在 recent_action, recent_search, recent_news 中設為 NULL
     ======================= */
    -- recent_action
  	UPDATE recent_action ra
  	LEFT JOIN user_view           uv   ON uv.view_id   = ra.view_id
  	LEFT JOIN user_share          us   ON us.share_id  = ra.share_id
  	LEFT JOIN user_search         ux   ON ux.search_id = ra.search_id
  	LEFT JOIN user_bookmark       ub   ON ub.bookmark_id = ra.bookmark_id
  	LEFT JOIN user_comment        uc   ON uc.comment_id  = ra.comment_id
  	LEFT JOIN user_score          usco ON usco.score_id  = ra.score_id
  	SET ra.view_id      = CASE WHEN uv.view_id   IS NOT NULL AND uv.created_at   < NOW() - INTERVAL 36 HOUR THEN NULL ELSE ra.view_id END,
      	ra.share_id     = CASE WHEN us.share_id  IS NOT NULL AND us.created_at   < NOW() - INTERVAL 36 HOUR THEN NULL ELSE ra.share_id END,
      	ra.search_id    = CASE WHEN ux.search_id IS NOT NULL AND ux.created_at   < NOW() - INTERVAL 36 HOUR THEN NULL ELSE ra.search_id END,
      	ra.bookmark_id  = CASE WHEN ub.bookmark_id IS NOT NULL AND ub.created_at < NOW() - INTERVAL 36 HOUR THEN NULL ELSE ra.bookmark_id END,
      	ra.comment_id   = CASE WHEN uc.comment_id  IS NOT NULL AND uc.created_at  < NOW() - INTERVAL 36 HOUR THEN NULL ELSE ra.comment_id END,
      	ra.score_id     = CASE WHEN usco.score_id IS NOT NULL AND usco.created_at < NOW() - INTERVAL 36 HOUR THEN NULL ELSE ra.score_id END;

  	DELETE FROM recent_action
    ...
    -- recent_search
    DELETE rs
    FROM recent_search AS rs
    JOIN user_search_record AS usr ON rs.record_id = usr.record_id
    WHERE usr.created_at < NOW() - INTERVAL 36 HOUR;
    
    -- recent_news
    DELETE rn
    FROM recent_news AS rn
    JOIN news_data AS nd ON rn.news_id = nd.news_id
    WHERE nd.created_at < NOW() - INTERVAL 36 HOUR;
...
CREATE TRIGGER `caculate_heat (bi_channel_data)` BEFORE INSERT ON `channel_data` FOR EACH ROW BEGIN
    -- 定義參數
    DECLARE v_view, v_share, v_search, v_bookmark, v_comment, v_score FLOAT DEFAULT 1.0;
    DECLARE v_recent_view, v_recent_share, v_recent_search, v_recent_bookmark, v_recent_comment, v_recent_score FLOAT DEFAULT 1.0;
    ...
    SELECT adjust_value INTO v_recent_score		FROM value_adjust WHERE adjust_type = 'recent_score'		LIMIT 1;
    ...
    COALESCE(NEW.total_score - COALESCE(3 * NEW.total_rater, 0), 0) * v_score + 
    ...
    COALESCE(NEW.total_recent_score - COALESCE(3 * NEW.total_rater, 0), 0) * v_recent_score;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `caculate_heat (bu_channel_data)` BEFORE UPDATE ON `channel_data` FOR EACH ROW BEGIN
    ...
    IF NOT (
        ...
        (NEW.total_recent_score <=> OLD.total_recent_score) AND
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `change_relation (bu_keyword)` BEFORE UPDATE ON `keyword_data` FOR EACH ROW IF NOT (
    NEW.keyword_relation_id <=> OLD.keyword_relation_id
)
THEN
	-- OLD keyword_relation_id 減掉
    UPDATE keyword_relation
    SET total_search_heat =  GREATEST(CAST(total_search_heat AS SIGNED) - CAST(NEW.total_search_heat AS SIGNED), 0)
    WHERE keyword_relation_id = OLD.keyword_relation_id;
    
    -- NEW keyword_relation_id 新增
    UPDATE keyword_relation
...
CREATE TRIGGER `update_relation_heat (au_keyword_data)` AFTER UPDATE ON `keyword_data` FOR EACH ROW IF NOT (
    ...
    UPDATE keyword_relation
    SET total_search_heat = GREATEST(CAST(total_search_heat AS SIGNED) + CAST(NEW.total_search_heat AS SIGNED) - CAST(OLD.total_search_heat AS SIGNED), 0)
    WHERE keyword_relation_id = NEW.keyword_relation_id;
END IF
$$
DELIMITER ;
...
THEN
	-- OLD relation_id 減掉
    UPDATE relation_data
    SET total_news_heat = GREATEST(CAST(total_news_heat AS SIGNED) - CAST(NEW.total_heat AS SIGNED), 0)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_news_data)` AFTER INSERT ON `news_data` FOR EACH ROW BEGIN
	-- HEAT update
	UPDATE relation_data
    ...
    -- RECENT update
    INSERT INTO recent_news (news_id)
    VALUES (NEW.news_id);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (au_news_data)` AFTER UPDATE ON `news_data` FOR EACH ROW BEGIN
    -- HEAT update
    IF NOT (
        NEW.total_heat <=> OLD.total_heat
    )
    ...
    -- RECENT update
    INSERT IGNORE INTO recent_news (news_id)
    VALUES (NEW.news_id);
END
$$
DELIMITER ;
...
--
-- 傾印資料表的資料 `permission_level`
--

INSERT INTO `permission_level` (`level_id`, `level_name`, `manage_data`, `add_manager`, `inherited_position`) VALUES
(0, 'User', 0, 0, 0),
(3, 'General Manage', 0, 0, 0),
(5, 'Data Manage', 1, 0, 0),
(7, 'Senior Manage', 1, 1, 0),
(10, 'Super Manage', 1, 1, 1);
...
CREATE TABLE `recent_action` (
  ...
  `bookmark_id` bigint(20) UNSIGNED DEFAULT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ad_user_bookmark)` AFTER DELETE ON `user_bookmark` FOR EACH ROW BEGIN
    -- news
    IF OLD.news_id IS NOT NULL
    THEN
        UPDATE news_data
        SET total_bookmark = GREATEST(CAST(total_bookmark AS SIGNED) - 1, 0)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_bookmark)` AFTER INSERT ON `user_bookmark` FOR EACH ROW BEGIN
    -- news
    IF NOT(
        NEW.news_id <=> NULL
    ) THEN
    ...
    -- RECENT update
    UPDATE recent_action
    SET bookmark_id = NEW.bookmark_id
    WHERE bookmark_id IS NULL
    LIMIT 1;

    IF ROW_COUNT() = 0 THEN
        INSERT INTO recent_action (bookmark_id)
        VALUES (NEW.bookmark_id);
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (au_user_bookmark)` AFTER UPDATE ON `user_bookmark` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_bookmark)` BEFORE INSERT ON `user_bookmark` FOR EACH ROW IF (
    -- 1. DATATYPE exactly one
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_bookmark)` BEFORE UPDATE ON `user_bookmark` FOR EACH ROW BEGIN
	-- 1. COLUMN limit
    IF (NOT (NEW.bookmark_id			 <=> OLD.bookmark_id) OR
        NOT (NEW.user_id                 <=> OLD.user_id)) 
    THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Only DataType, groupcustomize_id can be updated';
    END IF;
	
    -- 2. DATATYPE limit
	SET @old_type =
        CASE
            WHEN OLD.news_id IS NOT NULL THEN 'news'
            WHEN OLD.channel_id IS NOT NULL THEN 'channel'
            WHEN OLD.eventsorting_id IS NOT NULL THEN 'eventsorting'
            WHEN OLD.multipleperspectives_id IS NOT NULL THEN 'multipleperspectives'
        END;
        
	SET @new_type =
        CASE
            WHEN NEW.news_id IS NOT NULL THEN 'news'
            WHEN NEW.channel_id IS NOT NULL THEN 'channel'
            WHEN NEW.eventsorting_id IS NOT NULL THEN 'eventsorting'
            WHEN NEW.multipleperspectives_id IS NOT NULL THEN 'multipleperspectives'
        END;
        
	-- a. DATATYPE cannot change
	IF @old_type <> @new_type THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Type switch is not allowed: keep the same target column (news / channel / eventsorting / multipleperspectives).';
    END IF;
	
    -- b. exactly one of DATATYPE
    IF (
        ( (NEW.news_id IS NOT NULL) + 
          (NEW.channel_id IS NOT NULL) + 
          (NEW.eventsorting_id IS NOT NULL) + 
...
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ad_user_comment)` AFTER DELETE ON `user_comment` FOR EACH ROW BEGIN
    -- news
    IF OLD.news_id IS NOT NULL
    THEN
        UPDATE news_data
        SET total_bookmark = GREATEST(CAST(total_comment AS SIGNED) - 1, 0)
        WHERE news_id = OLD.news_id;
    END IF;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_comment)` AFTER INSERT ON `user_comment` FOR EACH ROW BEGIN
    -- news
    IF NOT(
        NEW.news_id <=> NULL
    ) THEN
    ...
    IF ROW_COUNT() = 0 THEN
        INSERT INTO recent_action (comment_id)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_comment)` BEFORE INSERT ON `user_comment` FOR EACH ROW IF (
    -- 1. DATATYPE exactly one
    ( (NEW.news_id IS NOT NULL) + 
      (NEW.channel_id IS NOT NULL) + 
      (NEW.eventsorting_id IS NOT NULL) + 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_comment)` BEFORE UPDATE ON `user_comment` FOR EACH ROW BEGIN
	-- 1. COLUMN limit
    IF (NOT (NEW.comment_id				 <=> OLD.comment_id) OR
        NOT (NEW.user_id                 <=> OLD.user_id) OR
        NOT (NEW.news_id                 <=> OLD.news_id) OR
        NOT (NEW.channel_id              <=> OLD.channel_id) OR
        NOT (NEW.eventsorting_id         <=> OLD.eventsorting_id) OR
        NOT (NEW.multipleperspectives_id <=> OLD.multipleperspectives_id)) 
    THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Only anonymous_id, comment_text can be updated';
    END IF;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_location)` BEFORE INSERT ON `user_location` FOR EACH ROW IF (
    -- LOCATIONTYPE exactly one
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_location)` BEFORE UPDATE ON `user_location` FOR EACH ROW BEGIN
	-- 1. COLUMN limit
    IF NOT (NEW.user_id <=> OLD.user_id)
    THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Only LocationType can be updated';
    END IF;

	-- 2. LOCATIONTYPE limit
    IF (
		( (NEW.region_id IS NOT NULL) + 
         (NEW.country_id IS NOT NULL) + 
         (NEW.state_id IS NOT NULL)
...
CREATE TABLE `user_profile` (
  ...
  `last_country` int(11) DEFAULT 216,
...
CREATE TABLE `user_score` (
  ...
  `eventsorting_id` bigint(20) UNSIGNED DEFAULT NULL,
  `multipleperspectives_id` bigint(20) UNSIGNED DEFAULT NULL,
  `target_score` tinyint(1) UNSIGNED NOT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ad_user_score)` AFTER DELETE ON `user_score` FOR EACH ROW BEGIN
    -- news
    IF OLD.news_id IS NOT NULL
    THEN
        UPDATE news_data
        SET total_score = GREATEST(CAST(total_score AS SIGNED) - CAST(OLD.target_score AS SIGNED), 0),
        	total_rater = GREATEST(CAST(total_rater AS SIGNED) - 1, 0)
        WHERE news_id = OLD.news_id;
    END IF;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_score)` AFTER INSERT ON `user_score` FOR EACH ROW BEGIN
    -- news
    IF NOT(
        NEW.news_id <=> NULL
    ) THEN
        UPDATE news_data
        SET total_score = total_score + NEW.target_score,
        	total_rater = total_rater + 1
        WHERE news_id = NEW.news_id;
    END IF;
    ...
    -- RECENT update
    UPDATE recent_action
    SET score_id = NEW.score_id
    WHERE score_id IS NULL
    LIMIT 1;

    IF ROW_COUNT() = 0 THEN
        INSERT INTO recent_action (score_id)
        VALUES (NEW.score_id);
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `heat_and_recent (au_user_score)` AFTER UPDATE ON `user_score` FOR EACH ROW BEGIN
	DECLARE v_exists INT DEFAULT 0;
    
    -- news
    IF (OLD.news_id IS NOT NULL AND 
		NOT (OLD.target_score <=> NEW.target_score))
    THEN
        UPDATE news_data
        SET total_score = GREATEST(CAST(total_score AS SIGNED) + CAST(NEW.target_score AS SIGNED) - CAST(OLD.target_score AS SIGNED), 0)
        WHERE news_id = OLD.news_id;
    END IF;
    
	-- channel
    IF (OLD.channel_id IS NOT NULL AND 
		NOT (OLD.target_score <=> NEW.target_score))
    THEN
        UPDATE channel_data
        SET total_score = GREATEST(CAST(total_score AS SIGNED) + CAST(NEW.target_score AS SIGNED) - CAST(OLD.target_score AS SIGNED), 0)
    ...
    -- eventsorting
    IF (OLD.eventsorting_id IS NOT NULL AND 
		NOT (OLD.target_score <=> NEW.target_score))
    THEN
        UPDATE eventsorting_data
        SET total_score = GREATEST(CAST(total_score AS SIGNED) + CAST(NEW.target_score AS SIGNED) - CAST(OLD.target_score AS SIGNED), 0)
	...
	-- multipleperspectives
    IF (OLD.multipleperspectives_id IS NOT NULL AND 
		NOT (OLD.target_score <=> NEW.target_score))
    THEN
        UPDATE multipleperspectives_data
        SET total_score = GREATEST(CAST(total_score AS SIGNED) + CAST(NEW.target_score AS SIGNED) - CAST(OLD.target_score AS SIGNED), 0)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_score)` BEFORE INSERT ON `user_score` FOR EACH ROW IF (
    -- DATATYPE exactly one
    ( (NEW.news_id IS NOT NULL) + 
      (NEW.channel_id IS NOT NULL) + 
      (NEW.eventsorting_id IS NOT NULL) + 
      (NEW.multipleperspectives_id IS NOT NULL)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_score)` BEFORE UPDATE ON `user_score` FOR EACH ROW BEGIN
	-- 1. COLUMN limit
    IF (NOT (NEW.score_id                <=> OLD.score_id) OR
        NOT (NEW.user_id                 <=> OLD.user_id) OR
        NOT (NEW.news_id                 <=> OLD.news_id) OR
        NOT (NEW.channel_id              <=> OLD.channel_id) OR
        NOT (NEW.eventsorting_id         <=> OLD.eventsorting_id) OR
    ...
    THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Only target_score can be updated';
    END IF;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_search)` AFTER INSERT ON `user_search` FOR EACH ROW BEGIN
    -- news
    IF NOT(
        NEW.news_id <=> NULL
    ) THEN
	...
	-- RECENT update
    UPDATE recent_action
    SET search_id = NEW.search_id
    WHERE search_id IS NULL
    LIMIT 1;

    IF ROW_COUNT() = 0 THEN
        INSERT INTO recent_action (search_id)
        VALUES (NEW.search_id);
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_search)` BEFORE INSERT ON `user_search` FOR EACH ROW IF (
    -- DATATYPE exactly one
    ( (NEW.news_id IS NOT NULL) + 
      (NEW.channel_id IS NOT NULL) + 
      (NEW.eventsorting_id IS NOT NULL) + 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_search)` BEFORE UPDATE ON `user_search` FOR EACH ROW SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'UPDATE is disabled on news.user_search'
$$
DELIMITER ;
...
--
-- 觸發器 `user_search_record`
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_search_record)` AFTER INSERT ON `user_search_record` FOR EACH ROW BEGIN
    -- HEAT update
    UPDATE keyword_data
    SET total_search = total_search + 1
    ...
    -- RECENT update
    INSERT INTO recent_search (record_id)
    VALUES (NEW.record_id);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (au_user_search_record)` AFTER UPDATE ON `user_search_record` FOR EACH ROW SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'UPDATE is disabled on news.user_search_record'
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_share)` AFTER INSERT ON `user_share` FOR EACH ROW BEGIN
    -- news
    IF NOT(
        NEW.news_id <=> NULL
    ) THEN
    ...
    -- RECENT update
    UPDATE recent_action
    SET share_id = NEW.share_id
    WHERE share_id IS NULL
    LIMIT 1;

    IF ROW_COUNT() = 0 THEN
        INSERT INTO recent_action (share_id)
        VALUES (NEW.share_id);
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_share)` BEFORE INSERT ON `user_share` FOR EACH ROW IF (
    -- DATATYPE exactly one
    ( (NEW.news_id IS NOT NULL) + 
      (NEW.channel_id IS NOT NULL) + 
      (NEW.eventsorting_id IS NOT NULL) + 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_share)` BEFORE UPDATE ON `user_share` FOR EACH ROW SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'UPDATE is disabled on news.user_view'
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `heat_and_recent (ai_user_view)` AFTER INSERT ON `user_view` FOR EACH ROW BEGIN
    -- news
    IF NOT(
        NEW.news_id <=> NULL
    ) THEN
    ...
    -- RECENT update
    UPDATE recent_action
    SET view_id = NEW.view_id
    WHERE view_id IS NULL
    LIMIT 1;
    
    IF ROW_COUNT() = 0 THEN
        INSERT INTO recent_action (view_id)
        VALUES (NEW.view_id);
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_insert (bi_user_view)` BEFORE INSERT ON `user_view` FOR EACH ROW IF (
    -- DATATYPE exactly one
    ( (NEW.news_id IS NOT NULL) + 
      (NEW.channel_id IS NOT NULL) + 
      (NEW.eventsorting_id IS NOT NULL) + 
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `limit_update (bu_user_view)` BEFORE UPDATE ON `user_view` FOR EACH ROW SIGNAL SQLSTATE '45000'
SET MESSAGE_TEXT = 'UPDATE is disabled on news.user_view'
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `reset_heat_totals (au_value_adjust)` AFTER UPDATE ON `value_adjust` FOR EACH ROW BEGIN
  CALL sp_reset_heat_totals();
END
$$
DELIMITER ;
...
-- 資料表索引 `recent_action`
--
ALTER TABLE `recent_action`
  ADD UNIQUE KEY `recent_action_ibfk_1` (`view_id`) USING BTREE,
  ADD UNIQUE KEY `recent_action_ibfk_2` (`share_id`) USING BTREE,
  ADD UNIQUE KEY `recent_action_ibfk_3` (`search_id`) USING BTREE,
  ADD UNIQUE KEY `recent_action_ibfk_4` (`bookmark_id`) USING BTREE,
  ADD UNIQUE KEY `recent_action_ibfk_5` (`comment_id`) USING BTREE,
  ADD UNIQUE KEY `recent_action_ibfk_6` (`score_id`) USING BTREE;
...
ALTER TABLE `recent_search`
  ADD UNIQUE KEY `keyword_id` (`record_id`);
...
ALTER TABLE `user_bookmark`
  ...
  ADD KEY `multipleperspectives_id` (`multipleperspectives_id`),
  ADD KEY `idx_ub_news_created` (`news_id`,`created_at`),
  ADD KEY `idx_ub_channel_created` (`channel_id`,`created_at`),
  ADD KEY `idx_ub_event_created` (`eventsorting_id`,`created_at`),
  ADD KEY `idx_ub_multi_created` (`multipleperspectives_id`,`created_at`);
...
ALTER TABLE `user_comment`
  ...
  ADD KEY `user_id_3` (`user_id`,`channel_id`),
  ADD KEY `user_id_4` (`user_id`,`eventsorting_id`),
  ADD KEY `user_id_5` (`user_id`,`multipleperspectives_id`),
  ADD KEY `idx_uc_news_created` (`news_id`,`created_at`),
  ADD KEY `idx_uc_channel_created` (`channel_id`,`created_at`),
  ADD KEY `idx_uc_event_created` (`eventsorting_id`,`created_at`),
  ADD KEY `idx_uc_multi_created` (`multipleperspectives_id`,`created_at`);
...
ALTER TABLE `user_profile`
  ...
  ADD KEY `user_profile_ibfk_1` (`last_country`),
...
ALTER TABLE `user_score`
  ...
  ADD KEY `eventsorting_id` (`eventsorting_id`),
  ADD KEY `multipleperspectives_id` (`multipleperspectives_id`),
  ADD KEY `idx_usco_news_created` (`news_id`,`created_at`),
  ADD KEY `idx_usco_channel_created` (`channel_id`,`created_at`),
  ADD KEY `idx_usco_event_created` (`eventsorting_id`,`created_at`),
  ADD KEY `idx_usco_multi_created` (`multipleperspectives_id`,`created_at`);
...
ALTER TABLE `user_search_record`
  ...
  ADD KEY `keyword_id` (`keyword_id`),
  ADD KEY `user_id` (`user_id`,`keyword_id`) USING BTREE;
...
ALTER TABLE `user_share`
  ...
  ADD KEY `eventsorting_id` (`eventsorting_id`),
  ADD KEY `multipleperspectives_id` (`multipleperspectives_id`),
  ADD KEY `idx_us_news_created` (`news_id`,`created_at`),
  ADD KEY `idx_us_channel_created` (`channel_id`,`created_at`),
  ADD KEY `idx_us_event_created` (`eventsorting_id`,`created_at`),
  ADD KEY `idx_us_multi_created` (`multipleperspectives_id`,`created_at`);
...
ALTER TABLE `user_view`
  ...
  ADD KEY `eventsorting_id` (`eventsorting_id`),
  ADD KEY `multipleperspectives_id` (`multipleperspectives_id`),
  ADD KEY `idx_uv_news_created` (`news_id`,`created_at`),
  ADD KEY `idx_uv_channel_created` (`channel_id`,`created_at`),
  ADD KEY `idx_uv_event_created` (`eventsorting_id`,`created_at`),
  ADD KEY `idx_uv_multi_created` (`multipleperspectives_id`,`created_at`);
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `user_search`
--
ALTER TABLE `user_search`
  MODIFY `search_id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `user_search_record`
--
ALTER TABLE `user_search_record`
...
--
-- 資料表的限制式 `recent_action`
--
ALTER TABLE `recent_action`
  ADD CONSTRAINT `recent_action_ibfk_1` FOREIGN KEY (`view_id`) REFERENCES `user_view` (`view_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `recent_action_ibfk_2` FOREIGN KEY (`share_id`) REFERENCES `user_share` (`share_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `recent_action_ibfk_3` FOREIGN KEY (`search_id`) REFERENCES `user_search` (`search_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `recent_action_ibfk_4` FOREIGN KEY (`bookmark_id`) REFERENCES `user_bookmark` (`bookmark_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `recent_action_ibfk_5` FOREIGN KEY (`comment_id`) REFERENCES `user_comment` (`comment_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `recent_action_ibfk_6` FOREIGN KEY (`score_id`) REFERENCES `user_score` (`score_id`) ON DELETE SET NULL ON UPDATE CASCADE;

--
-- 資料表的限制式 `recent_news`
--
ALTER TABLE `recent_news`
  ADD CONSTRAINT `recent_news_ibfk_1` FOREIGN KEY (`news_id`) REFERENCES `news_data` (`news_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `recent_search`
--
ALTER TABLE `recent_search`
  ADD CONSTRAINT `recent_search_ibfk_1` FOREIGN KEY (`record_id`) REFERENCES `user_search_record` (`record_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
-- 資料表的限制式 `user_location`
--
ALTER TABLE `user_location`
  ADD CONSTRAINT `user_location_ibfk_1` FOREIGN KEY (`region_id`) REFERENCES `location_regions` (`region_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `user_location_ibfk_2` FOREIGN KEY (`country_id`) REFERENCES `location_countries` (`country_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `user_location_ibfk_3` FOREIGN KEY (`state_id`) REFERENCES `location_states` (`state_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `user_profile`
--
ALTER TABLE `user_profile`
  ADD CONSTRAINT `user_profile_ibfk_1` FOREIGN KEY (`last_country`) REFERENCES `location_countries` (`country_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `user_profile_ibfk_2` FOREIGN KEY (`user_level`) REFERENCES `permission_level` (`level_id`) ON UPDATE CASCADE;
...
--
ALTER TABLE `user_search`
  ADD CONSTRAINT `user_search_ibfk_2` FOREIGN KEY (`news_id`) REFERENCES `news_data` (`news_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `user_search_ibfk_3` FOREIGN KEY (`channel_id`) REFERENCES `channel_data` (`channel_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `user_search_ibfk_4` FOREIGN KEY (`eventsorting_id`) REFERENCES `eventsorting_data` (`eventsorting_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `user_search_ibfk_5` FOREIGN KEY (`multipleperspectives_id`) REFERENCES `multipleperspectives_data` (`multipleperspectives_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- 資料表的限制式 `user_search_record`
--
ALTER TABLE `user_search_record`
  ADD CONSTRAINT `user_search_record_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_profile` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
-- 事件
--
CREATE DEFINER=`root`@`localhost` EVENT `ev_rollup_recent_actions_hourly` ON SCHEDULE EVERY 1 HOUR STARTS '2025-10-01 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL sp_rollup_recent()$$
