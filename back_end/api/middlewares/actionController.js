...
async function searchUserAction (req, res, next) {
    ...
    */
    ...
    // 特殊處理 bookmark 查詢 - 需要 JOIN 對應的 data 表來獲取完整資訊
    if (actionType === 'bookmark') {
        let joinTable = '';
        let selectFields = '';

        if (dataType === 'news') {
            joinTable = 'news_data nd';
            selectFields = `
                ub.bookmark_id,
                ub.user_id,
                ub.news_id,
                ub.groupcustomize_id,
                ub.created_at,
                nd.news_title,
                nd.origin_url,
                nd.news_date AS publish_date,
                cd.channel_name,
                id.image_origin_url AS cover_image_url,
                id.image_text AS cover_image_alt
            `;
            sql = `
                SELECT ${selectFields}
                FROM user_bookmark ub
                JOIN ${joinTable} ON ub.news_id = nd.news_id
                LEFT JOIN channel_data cd ON nd.channel_id = cd.channel_id
                LEFT JOIN image_data id ON nd.cover_image = id.image_id
                WHERE ub.user_id = ? AND ub.news_id IS NOT NULL
                ORDER BY ub.created_at DESC
            `;
        } else if (dataType === 'channel') {
            joinTable = 'channel_data cd';
            selectFields = `
                ub.bookmark_id,
                ub.user_id,
                ub.channel_id,
                ...
                ub.created_at,
                cd.channel_name,
                cd.channel_description,
                cd.channel_logo_url
            `;
            sql = `
                SELECT ${selectFields}
                FROM user_bookmark ub
                JOIN ${joinTable} ON ub.channel_id = cd.channel_id
                WHERE ub.user_id = ? AND ub.channel_id IS NOT NULL
                ORDER BY ub.created_at DESC
            `;
        } else if (dataType === 'eventsorting') {
            joinTable = 'eventsorting_data ed';
            selectFields = `
                ub.bookmark_id,
                ub.user_id,
                ub.eventsorting_id,
                ub.groupcustomize_id,
                ub.created_at,
                ed.eventsorting_title,
                ed.eventsorting_description,
                ed.created_at AS event_date
            `;
            sql = `
                SELECT ${selectFields}
                FROM user_bookmark ub
                JOIN ${joinTable} ON ub.eventsorting_id = ed.eventsorting_id
                WHERE ub.user_id = ? AND ub.eventsorting_id IS NOT NULL
                ORDER BY ub.created_at DESC
            `;
        } else if (dataType === 'multipleperspectives') {
            joinTable = 'multipleperspectives_data md';
            selectFields = `
                ub.bookmark_id,
                ub.user_id,
                ub.multipleperspectives_id,
                ub.groupcustomize_id,
                ub.created_at,
                md.multipleperspectives_title,
                md.multipleperspectives_description
            `;
            sql = `
                SELECT ${selectFields}
                FROM user_bookmark ub
                JOIN ${joinTable} ON ub.multipleperspectives_id = md.multipleperspectives_id
                WHERE ub.user_id = ? AND ub.multipleperspectives_id IS NOT NULL
                ORDER BY ub.created_at DESC
            `;
        }
        ...
        try {
            ...
            err.desc = "middlewares-searchUserAction(): database search error for bookmark with JOIN";
            return next(err);
        }
    }

    // 非 bookmark 的其他 action（score 等）保持原有查詢方式
    sql = `
        SELECT *
