...
def parse_channels(crawlerData, newsQueue):
    """解析頻道清單"""
    # push-fn
    from newsCrawler.utils import errorlog
    errorlog("push-fn", parse_channels)
    ...
    try:
        ...
        # set_data
        errorlog("set-data", "channels", channels)
        
    except Exception as e:
        print(f"✗ 解析頻道清單失敗: {str(e)}")
        errorlog("pop-fn")
        return []
...
def parse_news_list(soup, channel_url, crawlerData, newsQueue):
    """根據頻道 URL 選擇對應的解析器"""
    news_items = []
...
def get_news_information(news_items, channel_name, crawlerData, newsQueue, limit=20):
    """爬取新聞詳細資訊"""
    # push-fn
    from newsCrawler.utils import errorlog
    ...
    for idx, item in enumerate(news_items[:limit]):
        try:
            ...
            # set_data: 當前處理的新聞索引
            errorlog("set-data", "current_news_index", idx)
            ...
            # push to queue
            newsQueue.push_one(news_item)
            news_count += 1
            
            # 最後一筆時刪除索引記錄
            if idx == len(news_items[:limit]) - 1:
                errorlog("delete-data", "current_news_index")
            
        except Exception as e:
            print(f"  ✗ 爬取新聞失敗 ({title}): {str(e)}")
            # continue時也要更新errorlog
            if idx == len(news_items[:limit]) - 1:
                errorlog("delete-data", "current_news_index")
            continue
...
def crawl_channel_news(channel_name, channel_url, crawlerData, newsQueue, limit=20):
    """爬取單一頻道的新聞"""
    # push-fn
    from newsCrawler.utils import errorlog
    errorlog("push-fn", crawl_channel_news, [
        {"channel_name": channel_name},
    ...
    try:
        ...
        # 使用對應的解析器取得新聞列表
        news_items = parse_news_list(soup, channel_url, crawlerData, newsQueue)
        
        # set_data
        errorlog("set-data", "news_items_count", len(news_items))
        
        # 呼叫get_news_information前
        news_count = get_news_information(news_items, channel_name, crawlerData, newsQueue, limit)
        
        # delete_data (呼叫函數後)
        errorlog("delete-data", "news_items_count")
...
def crawl_setn(crawlerData, newsQueue):
    """主程式:爬取所有頻道的新聞"""
    # push-fn
    from newsCrawler.utils import errorlog
    errorlog("push-fn", crawl_setn)
    
    # 解析頻道清單
    print("正在解析頻道清單...")
    channels = parse_channels(crawlerData, newsQueue)
    print(f"找到 {len(channels)} 個頻道\n")
    
    # set_data
    errorlog("set-data", "total_channels", len(channels))
    errorlog("set-data", "channels_list", channels)
    ...
    for idx, channel in enumerate(channels):
        # set_data: 當前處理的頻道索引
        errorlog("set-data", "current_channel_index", idx)
        
        news_count = crawl_channel_news(
            channel["name"], 
            channel["url"], 
            crawlerData, 
            newsQueue, 
        ...
        print(f"{channel['name']}: {news_count} 篇新聞\n")
        
        # 最後一筆頻道處理完成
        if idx == len(channels) - 1:
            errorlog("delete-data", "current_channel_index")
            errorlog("delete-data", "channels_list")
    ...
    print(f"\n爬取完成!")
    print(f"總共爬取 {total_news} 篇新聞")
    print(f"已輸出 4_CHANNEL.json")
    
    # delete_data
    errorlog("delete-data", "total_channels")
...
def clean_errorlog():
    ...
    from newsCrawler.utils import errorlog
    # 清空整個errorlog
    errorlog("set-data", "fn", [])
    errorlog("set-data", "data", {})
    print("ErrorLog 已清空")
...
def run(crawler_data):
    ...
    ErrorLog(
        ...
        fn_map=[
            [crawl_setn, ["crawlerData", "newsQueue"]],
            [parse_channels, ["crawlerData", "newsQueue"]],
            [crawl_channel_news, ["channel_name", "channel_url", "crawlerData", "newsQueue", "limit"]],
            [get_news_information, ["news_items", "channel_name", "crawlerData", "newsQueue", "limit"]]
        ],
        init_data={
            "crawlerData": crawler_data,
            "newsQueue": newsQueue
        },
        entry=partial(crawl_setn, crawler_data, newsQueue)
    )
    
    # 執行主程式 (由ErrorLog自動執行entry)
    # crawl_setn(crawler_data, newsQueue)
